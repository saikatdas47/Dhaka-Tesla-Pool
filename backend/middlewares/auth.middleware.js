...
export const verifyPassenger = asyncHandler(async (request, _response, next) => {
  ...
  if (passenger.isDemo && !demoAccountsEnabled()) throw new ApiError(401, "Demo access is disabled.");
...
export const verifyDriver = asyncHandler(async (request, _response, next) => {
  ...
  if (driver.isDemo && !demoAccountsEnabled()) throw new ApiError(401, "Demo access is disabled.");
