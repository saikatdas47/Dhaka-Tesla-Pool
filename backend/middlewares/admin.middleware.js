...
export function checkAdminCredentials(username, password) {
  if (!adminConfigured())
    throw new ApiError(503, "Admin login is not configured.");
  return (
    sameSecret(String(username || ""), process.env.ADMIN_USERNAME) &&
    sameSecret(String(password || ""), process.env.ADMIN_PASSWORD)
  );
}

export function setAdminSession(response) {
  const token = jwt.sign(
    { role: "admin", tokenType: "access" },
