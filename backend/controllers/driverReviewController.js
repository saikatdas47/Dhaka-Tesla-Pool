...
function publicReview(review) {
  return {
    id: review.id,
    rideId: String(review.ride),
    passengerName:
      review.passengerName || review.passenger?.name || "Passenger",
    driverId: String(review.driver),
...
async function reviewsForDriver(driverId, page = 1) {
  const [reviews, total, average] = await Promise.all([
    DriverReview.find({ driver: driverId })
      .sort({ createdAt: -1 })
      .skip((page - 1) * 20)
      .limit(20)
      .populate("passenger", "name"),
    DriverReview.countDocuments({ driver: driverId }),
    DriverReview.aggregate([
  ...
  return {
    ...
    averageRating: average[0]?.rating ?? null,
  };
}
...
export const submitDriverReview = asyncHandler(async (request, response) => {
  ...
  if (!Number.isInteger(rating) || rating < 1 || rating > 5)
    throw new ApiError(400, "Choose 1 to 5 stars.");
  ...
  const ride = await RideRequest.findOne({
    ...
    status: "COMPLETED",
  }).populate("pool", "driver");
  if (!ride?.pool?.driver)
    throw new ApiError(
      403,
      "Only a Passenger who completed this driver's trip can review it.",
    );
  try {
    const review = await DriverReview.create({
      ride: ride.id,
      passenger: request.passenger.id,
      passengerName: ride.passengerName || request.passenger.name,
    ...
    if (error.code === 11000)
      throw new ApiError(409, "You already reviewed this ride.");
    throw error;
  }
});

export const myDriverReviews = asyncHandler(async (request, response) => {
  const reviews = await DriverReview.find({ passenger: request.passenger.id })
    .select("ride passenger passengerName driver rating comment createdAt")
    .populate("passenger", "name");
  response.json(
    new ApiResponse(
      200,
...
export const ownDriverReviews = asyncHandler(async (request, response) => {
  ...
  response.json(
    new ApiResponse(
      200,
      await reviewsForDriver(request.driver.id, page),
      "Driver reviews.",
    ),
  );
});
...
export const adminDriverReviews = asyncHandler(async (request, response) => {
  ...
  if (!(await Driver.exists({ _id: request.params.id })))
  ...
  response.json(
    new ApiResponse(
      200,
      await reviewsForDriver(request.params.id, page),
      "Driver reviews.",
    ),
  );
});
