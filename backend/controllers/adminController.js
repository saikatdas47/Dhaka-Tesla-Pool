...
export const listPassengers = asyncHandler(async (request, response) => {
  ...
  const fields = { name: "name", username: "username", email: "email", phone: "phone" };
  ...
  const [passengers, total] = await Promise.all([
    Passenger.find(filter).sort({ createdAt: -1 }).skip((page - 1) * 20).limit(20),
    Passenger.countDocuments(filter),
  ]);
  response.json(new ApiResponse(200, { passengers: passengers.map(passengerSummary), page, pages: Math.ceil(total / 20), total }, "Passenger accounts fetched."));
});

export const getPassengerForAdmin = asyncHandler(async (request, response) => {
  if (!mongoose.isValidObjectId(request.params.id)) throw new ApiError(400, "Invalid passenger ID.");
  const passenger = await Passenger.findById(request.params.id);
  if (!passenger) throw new ApiError(404, "Passenger not found.");
  response.json(new ApiResponse(200, { passenger: passengerSummary(passenger) }, "Passenger details fetched."));
});
...
export const getAdminFareSettings = asyncHandler(async (_request, response) => {
  response.json(new ApiResponse(200, await getFareSettings(), "Fare settings fetched."));
});

export const updateAdminFareSettings = asyncHandler(async (request, response) => {
  const { baseFarePaisa, perKmPaisa, sharedDiscountPercent } = request.body || {};
  if (!Number.isSafeInteger(baseFarePaisa) || baseFarePaisa < 0 || baseFarePaisa > 1000000 ||
      !Number.isSafeInteger(perKmPaisa) || perKmPaisa < 0 || perKmPaisa > 100000 ||
      !Number.isSafeInteger(sharedDiscountPercent) || sharedDiscountPercent < 0 || sharedDiscountPercent > 100) {
    throw new ApiError(400, "Use integer paisa: base 0–1000000, per km 0–100000, and discount 0–100%. ");
  }
  ...
  response.json(new ApiResponse(200, { baseFarePaisa: settings.baseFarePaisa, perKmPaisa: settings.perKmPaisa, sharedDiscountPercent: settings.sharedDiscountPercent, updatedAt: settings.updatedAt, configured: true }, "Fare settings saved. New rides use these rates."));
});
