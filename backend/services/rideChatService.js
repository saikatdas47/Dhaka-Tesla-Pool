...
export async function chatAccess(rideId, role, accountId) {
  ...
  let allowed = false;
  if (role === "passenger")
    allowed = String(ride.passenger) === String(accountId);
  else if (role === "driver")
...
export async function chatSnapshot(rideId, role, accountId) {
  ...
  const chat = await RideChat.findOne({
    ride: rideId,
    status: "open",
...
export async function sendRideMessage(rideId, role, accountId, input) {
  ...
  if (!text || text.length > 500)
    throw new ApiError(400, "Message must be 1–500 characters.");
  ...
  const chat = await RideChat.findOneAndUpdate(
    {
      ...
      status: "open",
      expiresAt: { $gt: sentAt },
      $expr: { $lt: [{ $size: "$messages" }, 200] },
    },
    {
      $push: {
        messages: { senderRole: role, sender: accountId, text, sentAt },
      },
    },
