...
export async function removeAccountAvatar(account, Model) {
  if (account.pendingAvatarFilename)
    throw new Error("Retry the saved image before removing the current photo.");
  ...
  if (!updated)
    throw new Error(
      "Photo changed while it was being removed. Refresh and try again.",
    );
  return updated;
}
...
export async function sendPendingAvatarToCloudinary(account, Model, role) {
  ...
  if (account.avatarPublicId) {
    const deleted = await deleteFromCloudinary(account.avatarPublicId);
    if (!["ok", "not found"].includes(deleted.result))
      throw new Error("Old image could not be removed.");
    ...
    if (cleared.matchedCount !== 1)
      throw new Error("Avatar record changed during removal.");
  }
  ...
  if (saved.matchedCount !== 1)
    throw new Error("Pending image record changed during upload.");
