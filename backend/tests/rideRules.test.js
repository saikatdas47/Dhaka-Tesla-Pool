...
test("Dhaka zones, fare and compatible shared routes", () => {
  ...
  const custom = fareQuote("Banani", "Mohakhali", 2, { baseFarePaisa: 6000, perKmPaisa: 2500, sharedDiscountPercent: 25 });
  assert.equal(custom.soloFarePaisa, 22000);
  assert.equal(custom.pooledFarePaisa, 16500);
  assert.deepEqual(custom.fareRule, { baseFarePaisa: 6000, perKmPaisa: 2500, sharedDiscountPercent: 25 });
  assert.equal(compatibleRoutes({ pickupArea: "Banani", destinationArea: "Mohakhali" }, { pickupArea: "Banani", destinationArea: "Gulshan 1" }), true);
  assert.equal(compatibleRoutes({ pickupArea: "Banani", destinationArea: "Mohakhali" }, { pickupArea: "Mirpur", destinationArea: "Uttara" }), false);
