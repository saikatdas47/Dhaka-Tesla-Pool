...
test("driver signup, role-separated login, username login and protected dashboard", async () => {
  ...
  try {
    ...
    Driver.findOne = ({ $or }) => ({
      select: async () => saved && $or.some((item) => item.email === saved.email || item.username === saved.username) ? saved : null,
    });
    ...
    const created = await request("/api/drivers/register", { method: "POST", body: JSON.stringify({ ...fields, registrationToken: "b".repeat(64) }) });
    ...
    const duplicate = await request("/api/drivers/register", { method: "POST", body: JSON.stringify(fields) });
    assert.equal(duplicate.response.status, 409);

    const emailLogin = await request("/api/drivers/login", { method: "POST", body: JSON.stringify({ identity: fields.email, password: fields.password }) });
    assert.equal(emailLogin.response.status, 200);
    const usernameLogin = await request("/api/drivers/login", { method: "POST", body: JSON.stringify({ identity: fields.username, password: fields.password }) });
    ...
    const refreshed = await request("/api/drivers/refresh-token", { method: "POST", headers: { Cookie: latestCookies } });
    assert.equal(refreshed.response.status, 200);
    const rotatedCookies = refreshed.response.headers.getSetCookie().map((value) => value.split(";")[0]).join("; ");
    const reused = await request("/api/drivers/refresh-token", { method: "POST", headers: { Cookie: latestCookies } });
    assert.equal(reused.response.status, 401);

    const current = await request("/api/drivers/me", { headers: { Cookie: cookie } });
    ...
    const wrongRole = await request("/api/drivers/me", { headers: { Authorization: `Bearer ${passengerToken}` } });
    assert.equal(wrongRole.response.status, 401);

    const logout = await request("/api/drivers/logout", { method: "POST", headers: { Cookie: rotatedCookies } });
    assert.equal(logout.response.status, 200);
    const revoked = await request("/api/drivers/refresh-token", { method: "POST", headers: { Cookie: rotatedCookies } });
