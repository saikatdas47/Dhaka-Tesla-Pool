...
test("email OTP has cooldown, attempt limit, role binding, and one-use registration token", async () => {
  ...
  try {
    ...
    EmailOtp.findOne = async ({ role, email }) =>
      record?.role === role && record?.email === email ? record : null;
    ...
    EmailOtp.updateOne = async (filter, update) => {
      if (
        ...
        filter.otpHash !== record.otpHash
      )
        return { matchedCount: 0 };
      Object.assign(record, update.$set);
      return { matchedCount: 1 };
    };
    EmailOtp.deleteOne = async () => {
      record = null;
    ...
    EmailOtp.findOneAndDelete = async (filter) => {
      if (
        ...
        filter.registrationTokenHash !== record.registrationTokenHash ||
        record.expiresAt <= new Date()
      )
        return null;
      const used = record;
    ...
    await assert.rejects(
      sendRegistrationOtp("passenger", sent.email, async () => {}),
      { statusCode: 429 },
    );
    await assert.rejects(
      verifyRegistrationOtp("driver", sent.email, sent.otp),
      { statusCode: 400 },
    );
    await assert.rejects(
      verifyRegistrationOtp("passenger", sent.email, "000000"),
      { statusCode: 400 },
    );

    const token = await verifyRegistrationOtp(
      "passenger",
      sent.email,
      sent.otp,
    );
    ...
    await assert.rejects(
      consumeRegistrationToken("passenger", sent.email, token),
      { statusCode: 400 },
    );
    ...
    for (let attempt = 1; attempt <= 5; attempt += 1) {
      await assert.rejects(
        verifyRegistrationOtp("passenger", sent.email, "000000"),
        { statusCode: attempt === 5 ? 429 : 400 },
      );
    }
    await assert.rejects(
      verifyRegistrationOtp("passenger", sent.email, sent.otp),
      { statusCode: 400 },
    );
  } finally {
