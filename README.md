...
Fare is stored as integer **paisa** (100 paisa = ৳1). The deterministic estimate uses approximate distance between fixed area centres: `(৳50 + ৳20 × rounded kilometres) × requested seats`. A pool with at least two passenger requests gets 20% off each passenger's fare; otherwise the solo estimate applies. For one seat, Banani → Mohakhali and Banani → Gulshan 1 each estimate ৳90 solo or ৳72 when shared. The final fare is frozen on completion. Cash becomes **due** at completion and the assigned driver marks it **paid** when received. Simulated TeslaPay becomes **paid** at completion without moving real money or maintaining a wallet balance. Method, status, amount, payment time, and events remain in the ride history. Older rides without payment data show **not recorded**.
