...
async function request(role, path, options = {}) {
  const response = await roleFetch(role, `/api/rides${path}`, { ...options, headers: { "Content-Type": "application/json", ...options.headers } });
...
async function updateDriverAvailability(body) {
  const response = await roleFetch("driver", "/api/drivers/availability", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
...
export function DriverRides({ driver, onDriverUpdated }) {
  ...
  const [error, setError] = useState("");
  async function refresh() {
    const [available, current, accountResponse] = await Promise.all([request("driver", "/offers"), request("driver", "/driver/current"), roleFetch("driver", "/api/drivers/me")]);
...
export function RideHistory({ role }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    request("passenger", "/reviews/mine").then(({ reviews: list }) => {
      if (active) setReviews(Object.fromEntries(list.map((review) => [review.rideId, review])));
