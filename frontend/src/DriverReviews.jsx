...
export default function DriverReviews({ endpoint }) {
  ...
  useEffect(() => {
    ...
    (endpoint.startsWith("/api/admin/")
      ? fetch(`${endpoint}?page=${page}`, { credentials: "same-origin" })
      : roleFetch("driver", `${endpoint}?page=${page}`)
    )
      .then(async (response) => {
        const body = await response.json();
        if (!response.ok)
          throw new Error(body.message || "Reviews could not load.");
        return body.data;
      })
      .then((value) => {
  ...
  return (
    <section className="info-card driver-reviews">
      <div className="driver-reviews-head">
        <div>
          <span className="card-kicker">PASSENGER FEEDBACK</span>
          <h2>Driver reviews</h2>
        </div>
        {data && (
          <span className="driver-reviews-summary">
      ...
      {data?.reviews.map((review) => (
        <article className="driver-review" key={review.id}>
